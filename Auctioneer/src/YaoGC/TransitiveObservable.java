...
public class TransitiveObservable {
    ...
    List<TransitiveObserver> observers = new ArrayList<TransitiveObserver>(); //列表observers全都为被观察者
   
    boolean changed = false;
   
    List<Socket> exports = new ArrayList<Socket>(); //列表exports全都为Socket类型
	...
	synchronized (this) { //同一时刻最多只有一个线程执行该段代码
		...
		observers.add(observer); //将观察者oberver加入被观察者obervers
		exports.add(sock); //
	}
    }
