...
public class TestAuctioneer {
	...
	private static void ReceivePublicKey() throws Exception {
		...
		System.out.println("paillier g = " + g);
		n = bClient.ReceiveBigInteger();

		System.out.println("paillier n = " + n);
	}
	...
	private static void ReceiveEncryptPrice() throws Exception {
		...
		System.out.println("已经收到" + nPerson + "个买家paillier加密后的价格:");
		for (int i = 0; i < nPerson; i++) {
			System.out.println("ePkPrice["+i+"] = " + ePkPrice[i]);
		}
	}
	...
	private static void GarbleCircuits() throws Exception {
		...
		Auctioneer auctionclient = new Auctioneer(rand, nPerson, IP);
		auctionclient.run();
	}

	AuctioneerWindow client;
