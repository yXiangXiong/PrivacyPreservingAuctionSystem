    ...
    protected void connectWires() throws Exception 
    {
    	...
    	for (int level = 0; level < outDegree-1; level++)
    	{
    		if (level == outDegree-2)
    		{
    			for (int x = 0; x < (1<<level); x++) 
    			{
    				((ADD_2L_Lplus1) subCircuits[i]).connectWiresToXY(inputWires, j, inputWires, j+1);
    				i++; j += 2;
    			}
    		}
    		else 
    		{
    			for (int x = 0; x < (1<<level); x++)
    			{
    				((ADD_2L_Lplus1) subCircuits[i]).connectWiresToXY(subCircuits[2*i+1].outputWires, 0, subCircuits[2*i+2].outputWires, 0);
    				i++;
    			}
	    	}
    	}
